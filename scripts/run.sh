...
function show_help {
  ...
  echo ""
  echo "Examples:"
  echo "  ./scripts/run.sh                           # Run with Pi defaults"
  echo "  ./scripts/run.sh --platform=osx            # Run in macOS mode"
  echo "  ./scripts/run.sh --input-dir=/path/to/dir  # Use custom input directory"
  echo "  ./scripts/run.sh --local                   # Run locally outside Docker"
}
...
if [ "$RUN_LOCAL" = true ]; then
  ...
  AUDIO_INPUT_DIR="$AUDIO_INPUT_DIR" PLATFORM="$PLATFORM" python3 "$PROJECT_ROOT/transcriber.py"
else
