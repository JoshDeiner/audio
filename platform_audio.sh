...
EOF
      ...
      DOCKER_AUDIO_ARGS="-v /tmp/pulseaudio.socket:/tmp/pulseaudio.socket -v $(pwd)/config/pulse-client.conf:/etc/pulse/client.conf -e PULSE_SERVER=unix:/tmp/pulseaudio.socket"
...
echo "DOCKER_AUDIO_ARGS=\"$DOCKER_AUDIO_ARGS\"" > config/audio_config.env
echo "AUDIO_DRIVER=$AUDIO_DRIVER" >> config/audio_config.env
