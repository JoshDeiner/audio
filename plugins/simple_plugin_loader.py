...
class SimplePluginLoader:
    ...
    def load_plugin(self, plugin_type: str, plugin_id: Optional[str] = None) -> Any:
        ...
        try:
            ...
            logger.info(f"Loaded builtin plugin: {plugin_id} for type {plugin_type}")
        ...
        try:
            ...
            # Instantiate the plugin
            logger.info(f"Loaded custom plugin: {plugin_id} for type {plugin_type}")
            return plugin_class(**plugin_config)
        except (ImportError, AttributeError) as e:
            logger.error(f"Failed to load plugin {plugin_id} for type {plugin_type}: {e}")
            raise ImportError(f"Plugin {plugin_id} for type {plugin_type} not found")
