...
async def controller_example(service_provider: ServiceProvider, args: Dict[str, str]):
...
def service_registration_example():
    ...
    container.register(
        ...
        factory=lambda c: MockPlatformService(c.resolve(IConfigurationManager)),
        lifetime=ServiceLifetime.SINGLETON
    )
...
def scoped_services_example(service_provider: ServiceProvider):
    ...
    print(f"Same instance? {root_scoped_service is request_scoped_service}")  # False
