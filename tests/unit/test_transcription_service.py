...
class TestFileTranscriptionUnit:
    ...
    def test_transcribe_file_called_correctly(self, mocker):
        ...
        mock_transcribe = mocker.patch.object(
            FileTranscriptionService, "transcribe_file", return_value="mocked text"
        )

        result = service.transcribe_file(file_path, model_size="tiny", language="en")

        mock_transcribe.assert_called_once_with(file_path, model_size="tiny", language="en")
