...
class TestAudioRecording:
    ...
    def mock_audio_recording(self):
        ...
        if not os.path.exists(test_file):
            dummy_wav_path = os.path.join(os.path.dirname(__file__), "..", "assets", "test_audio.wav")
    ...
    @pytest.mark.asyncio
    async def test_audio_pipeline_recording_with_mocked_audio(self, setup_environment, mock_audio_recording):
        ...
        config = {
            ...
            "output_path": os.path.join(setup_environment["output_dir"], "test_transcript.txt")
        }
        ...
        with patch.object(controller, '_record_audio_async', return_value=mock_audio_recording):
            # Run the pipeline
            try:
    ...
    @pytest.mark.asyncio
    async def test_audio_pipeline_quality_validation(self, setup_environment, mock_audio_recording):
        ...
        assert hasattr(services.audio_service, '_validate_audio_quality'), "Audio service missing quality validation method"
        
        # Try to validate an empty frame list (should fail)
        validation_result = services.audio_service._validate_audio_quality([])
        assert validation_result is False, "Empty frames should fail validation"
