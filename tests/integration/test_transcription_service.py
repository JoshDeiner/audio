...
def test_full_audio_pipeline() -> None:
    ...
    file_service.save(audio_data=(audio_data, sample_rate), file_path=temp_audio_file)
    ...
    similarity = difflib.SequenceMatcher(None, test_text.lower(), transcribed_text.lower()).ratio()
...
def test_text_file_input_pipeline(tmp_path: Path) -> None:
    ...
    trans_service = TranscriptionService()
    transcribed_text = trans_service.transcribe_audio(
        audio_file_path=str(audio_output_file),
        model_size="tiny"
    )
