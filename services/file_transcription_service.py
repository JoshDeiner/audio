...
class FileTranscriptionService:
    ...
    def _process_file(
        self, 
        args: Tuple[str, Optional[str], Optional[str]]
    ...
    def process_files_parallel(
        ...
        if not audio_files:
            ...
            print(f"{Fore.YELLOW}No audio files found in {input_dir}{Style.RESET_ALL}")
            return []
            
        logger.info(f"Found {len(audio_files)} audio files to process")
        print(f"{Fore.CYAN}Processing {len(audio_files)} audio files with {num_processes} processes...{Style.RESET_ALL}")
        ...
        try:
            ...
            process_args = [(file_path, model_size, language) for file_path in audio_files]
            ...
            with tqdm(total=len(results), desc="Processing audio files") as pbar:
                ...
                for r in results:
                    try:
                        ...
                        print(f"{Fore.RED}Error retrieving result: {e}{Style.RESET_ALL}")
                        pbar.update(1)
            ...
            print(f"{Fore.RED}Error in parallel processing: {e}{Style.RESET_ALL}")
        ...
        # Report results
        print(f"{Fore.GREEN}Successfully processed {len(successful_transcriptions)} files{Style.RESET_ALL}")
        if failed_files:
            print(f"{Fore.YELLOW}Failed to process {len(failed_files)} files{Style.RESET_ALL}")
