...
class AudioRecordingService(IAudioRecordingService):
    ...
    def _record_and_save_audio(
        ...
        try:
            ...
            if self._validate_audio_quality(frames):
                ...
                logger.warning(
                    "Audio quality validation failed, but continuing with processing"
                )
                print(
                    f"{Fore.YELLOW}Warning: Audio quality may be poor. Processing anyway...{Style.RESET_ALL}"
                )
    ...
    def _validate_audio_quality(self, frames: List[bytes]) -> bool:
        ...
        try:
            ...
            sample_frames = (
                frames if len(frames) < 10 else frames[:: len(frames) // 10]
            )
            levels = []
            ...
            if median_level < min_acceptable:
                logger.warning(
                    f"Audio level is too low: {median_level} < {min_acceptable}"
                )
                return False

            logger.info(
                f"Audio quality check passed with median level: {median_level}"
            )
            return True
